...
description: SEO keywords + algorithm updates + competitor pulse — chạy hàng tháng, date-stamped. Thay thế 3 skill riêng lẻ.
---
...
## 5. GPT Deep Synthesis (nếu có OPENAI_API_KEY)

**Sau khi hoàn thành sections 1–4**, gửi tất cả market intelligence data tới OpenAI o1 để:
- **Pattern Recognition:** Tìm mối liên hệ giữa SEO trends → algorithm changes → competitor moves
- **Opportunity Mapping:** Chỉ ra mục tiêu có SEO volume cao + competitor ít exploit + algorithm ưu tiên
- **Content Reprioritization:** Xếp hạng lại nội dung dự kiến theo feasibility + ROI dựa vào market data
- **Risk Signals:** Cảnh báo nếu có algorithm change nào có thể giảm traffic → cần backup strategy

**Output từ GPT:**
```json
{
  "market_patterns": ["pattern 1", "pattern 2", ...],
  "top_3_opportunities": [
    {
      "theme": "...",
      "seo_volume": "high/medium",
      "competitor_gap": "...",
      "algorithm_support": "...",
      "recommended_format": "..."
    }
...
**Prompt gửi cho GPT:**
```
Dữ liệu market intelligence [brand] [tháng]:

[Toàn bộ sections 1-4 ở trên]

Hãy:
1. Tìm 3 mô hình (pattern) nối liền giữa SEO → algorithm → competitor behavior
2. Chỉ ra 3 cơ hội tốt nhất (high SEO volume + low competitor + algorithm ưu tiên)
3. Xếp hạng lại content calendar dự kiến theo feasibility + ROI
4. Cảnh báo 2 rủi ro có thể xảy ra tuần tới
...
### OpenAI Deep Research (Khuyến Nghị)
...
**Khi chạy skill:**
- Nếu có `OPENAI_API_KEY` → tự động gọi OpenAI o1 sau khi hoàn thành section 4
- Nếu không có key → skip section 5, chỉ chạy WebSearch (như trước)

**Lợi ích:** GPT o1 kết hợp SEO + algorithm + competitor data → phát hiện opportunities người không thể thấy được. Tiết kiệm 1-2 tuần phân tích.
